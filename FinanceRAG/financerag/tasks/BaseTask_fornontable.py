...
# Query에서 Keyword 뽑기 
    ...
    def keyword_extraction_expansion(self, query: str) -> List[str]:
        ...
        response = self.client.chat.completions.create(
            ...
            messages=[{"role": "system", "content": 
                       ...
                       Return keywords only, nothing else.
                       """
                       },
    ...
    def query_extraction_expansion(self, query: str) -> List[str]:
        ...
        response = self.client.chat.completions.create(
            ...
            seed=42,
            messages=[{"role": "system", "content": 
                       """If there is a Ticker of the company, add the name of the company next to the Ticker.
                       If there is a full name of the company in the query, add the Ticker next to the full name of the company.
                       Also if there is a financial abbreviation, please explain it.
                       """
                       },
                    {"role": "user", "content": f"Input query: {query}"}],
            max_tokens=256,
        ...
        for q_id, query in tqdm(self.queries.items(), desc="Hybrid search", total=len(self.queries)):
            ...
            try:
                retrieved_docs_2 = (self.hybrid_retriever
                                    .search(query=query, query_type='vector') #top_k 삭제
                                    .to_pandas()
                                    )
            ...
            try: 
                ...
                retrieved_docs = (self.hybrid_retriever
                                ...
                                .rerank(reranker=self.reranker).limit(20).rerank(reranker=self.reranker).limit(10)
                                .to_pandas()
                    )
