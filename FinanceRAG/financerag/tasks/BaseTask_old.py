...
# Hybrid Search 구현
    def create_hybrid_retriever(self, mode: str = "overwrite", batch_size: int = 64) -> Dict[str, Dict[str, float]]:
        if (self.hybrid_retriever is None):
            ...
            # gpt 모델을 이용해 table 데이터를 자연어로 변환 
            self.Manage_Table()
        ...
        for q_id, query in tqdm(self.queries.items(), desc="Hybrid search", total=len(self.queries)):
            ...
            try:
                retrieved_docs = (self.hybrid_retriever
                            ...
                            .rerank(reranker=self.reranker).limit(10).rerank(reranker=self.reranker).limit(10)
                            .to_pandas()
                    )
            except Exception as e:
...
# Manage Tabular Data
# 테이블이 존재하는 데이터에 대해서 테이블을 우리가 아는 내용으로 바꾸는 과정
# 너무 오래걸림
    def Manage_Table(self):
        for cid, ctext in tqdm(self.corpus.items(), desc="Table extraction", total=len(self.corpus)):
            text = ctext['text']
            # use openai to extract keywords from the query
            response = self.client.chat.completions.create(
                model="gpt-4o-mini",
                seed=42,
                messages=[{"role": "system", "content": 
                        """Change Tabular data into natural language. Make sure not to lose any numbers and details in the table. 
                        Don't change anything not in table."""
                        },
                        {"role": "user", "content": f"Input query: {text}"}],
                max_tokens=256,
            ).choices[0].message.content
            ctext['text'] = response
        return True
