...
class BaseTask:
    ...
    def __init__(self, metadata: TaskMetadata, qrels = None):
        """
        ...
        self.openai_embedder = get_registry().get("openai").create(name="text-embedding-3-small")

        self.hybrid_retriever = None
        self.reranker = CohereReranker(api_key=os.getenv('CO_API_KEY'))#ColbertReranker() #CohereReranker(api_key=os.getenv('CO_API_KEY')) #JinaReranker(api_key=JINA_API_KEY)#
    ...
    def load_data(self):
        ...
        if (self.corpus is None) or (self.queries is None):
            ...
            if self.qrels != None:
                print("Now filtering queries.")
                print("Number of the queries before flitering is: ", len(queries))
                self.queries = {query["id"]: query["text"] for query in queries if query["id"] in self.qrels}
                print("Number of the queries after filtering is: ", len(self.queries))
                self.corpus = {
                    doc["id"]: {"title": doc["title"], "text": doc["text"]}
                    for doc in corpus
                }
            else:
                print("qrels == None. Unable to filter queries.")
...
# Hybrid Search 구현
    def create_hybrid_retriever(self, mode: str = "overwrite", batch_size: int = 64) -> Dict[str, Dict[str, float]]:
...
# Query에서 Keyword 뽑기 
    ...
    def keyword_extraction_expansion(self, query: str) -> List[str]:
        ...
        response = self.client.chat.completions.create(
            ...
            messages=[{"role": "system", "content": 
                       ...
                       Make sure the keyword is from finance domain. Avoid extracting generic terms.  
                       Return Finance keywords only, nothing else.
                       """
                       },
    ...
    @lru_cache(maxsize=1000)
    def query_extraction_expansion(self, query: str) -> List[str]:
        # use openai to extract keywords from the query
        response = self.client.chat.completions.create(
            model="gpt-4o-mini",
            seed=42,
            messages=[{"role": "system", "content": 
                       """You are given a query requiring numerical reasoning over hybrid data containing both textual and tabular content.
                          you only have to list up the information to look for. 

                          Below are two examples of queries.

                          Given query: How many kinds of period is the value of Customer deposits more than the 50% \of the total value of Customer deposits?
                          Part1: Value of Customer deposits. Total value of Customer deposits.
                          Part2: Kinds of period that the first value occupies more than 50%\ of the second value.

                          Given query: What is the sum of Passive for Equities, Active and other, and Company stock/ESOP in 2008? (in billion)
                          Part1: Passive for Equities in 2008. Active and other in 2008. Company stock/ESOP in 2008.
                          Part2: What is the sum of the three values?

                          Separate the given query and print only the content of Part1. Do not print "Part1". """
                       },
                    {"role": "user", "content": f"Input query: {query}"}],
            max_tokens=256,
        ).choices[0].message.content
    ...
    def hybrid_retrieve_rerank(self, top_k: int = 100, query_ids: Optional[List[str]] = None, alpha: float = 0.9, **kwargs) -> Dict[str, Dict[str, float]]:
        """
        ...
        for q_id, query in tqdm(self.queries.items(), desc="Hybrid search", total=len(self.queries)):
            ...
            #query = query + 'Keywords are ' + keywords
            query = self.query_extraction_expansion(query)
            query_kw = self.keyword_extraction_expansion(query)
            print(query)
            try:
                retrieved_docs_1 = (self.hybrid_retriever
                                    .search(query=query_kw, query_type='fts').limit(top_k) #limit(top_k)삭제
                                    .to_pandas()
                                    )
                #retrieved_docs_1['_rowid'] = range(len(retrieved_docs_1))
            except Exception as e:
                logger.warning(f"query {q_id} failed with full text search: {e}.")
                retrieved_docs_1 = pandas.DataFrame()
                _ = self._rename_score_column(retrieved_docs_1, "score")

            try:
                retrieved_docs_2 = (self.hybrid_retriever
                                    .search(query=query, query_type='vector').limit(top_k)  #.limit(top_k)top_k 삭제
                                    .to_pandas()
                                    )
                print(len(retrieved_docs_2))
                #retrieved_docs_2['_rowid'] = range(len(retrieved_docs_2))

            except Exception as e:
                logger.warning(f"query {q_id} failed with vector search: {e}.")
                retrieved_docs_2 = pandas.DataFrame()
                _ = self._rename_score_column(retrieved_docs_2, "score")
            

            try: 
                retrieved_docs_1['_rowid'] = range(len(retrieved_docs_1))
                retrieved_docs_2['_rowid'] = range(len(retrieved_docs_2))
                retrieved_docs = self.reranker.rerank_hybrid(query=query, fts_results=pa.Table.from_pandas(retrieved_docs_1),
                                                              vector_results=pa.Table.from_pandas(retrieved_docs_2)).to_pandas()
                ...
                retrieved_docs = (self.hybrid_retriever
                                .search(query=query, query_type='vector').limit(top_k)
                                .rerank(reranker=self.reranker).limit(30).rerank(reranker=self.reranker).limit(20)
                                .to_pandas()
                    )
            ...
            print("Vector search num = : ", vector_search_num)
        return results
    ...
    def save_results(self, top_k: int = 10, output_dir: Optional[str] = None) -> None:
        ...
        csv_file_path_score = os.path.join(output_dir, "results_score.csv")
        logger.info(f"Saving top {top_k} results to CSV file: {csv_file_path}")
        logger.info(f"Saving top {top_k} results with scores to CSV file: {csv_file_path_score}")
        ...
        if final_result is not None:
            ...
            with open(csv_file_path_score, mode="w", newline="") as csv_file:
                writer = csv.writer(csv_file)
                # Write the header to the CSV file
                writer.writerow(["query_id", "corpus_id", "score"])
                logger.info("Writing header ['query_id', 'corpus_id', 'score'] to CSV.")
                ...
                for q_id, doc_scores in final_result.items():
                    ...
                    for doc_id, score in sorted_docs:
                        writer.writerow([q_id, doc_id, score])

            logger.info(f"Top {top_k} results saved successfully to {csv_file_path_score}")
