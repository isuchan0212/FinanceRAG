...
from lancedb.rerankers import ColbertReranker, CohereReranker, JinaReranker, LinearCombinationReranker
...
class BaseTask:
    ...
    def __init__(self, metadata: TaskMetadata, qrels = None):
        ...
        self.reranker = LinearCombinationReranker(weight = 0.7) #CohereReranker(api_key=os.getenv('CO_API_KEY')) #JinaReranker(api_key=JINA_API_KEY)#
        print(self.reranker.weight)
    ...
    def keyword_extraction_expansion(self, query: str) -> List[str]:
        ...
        response = self.client.chat.completions.create(
            ...
            messages=[{"role": "system", "content": 
                       """Summarize the query first.
                       And from the original query, extract keywords from input query. 
    ...
    def hybrid_retrieve_rerank(self, top_k: int = 100, query_ids: Optional[List[str]] = None, **kwargs) -> Dict[str, Dict[str, float]]:
        ...
        for q_id, query in tqdm(self.queries.items(), desc="Hybrid search", total=len(self.queries)):
            ...
            try:
                ...
                retrieved_docs = (self.hybrid_retriever
                                .search(query=query, query_type='hybrid').limit(top_k) #.search(query = query, query_type = 'vector')
                                .rerank(reranker=self.reranker).limit(20).rerank(reranker=self.reranker).limit(10)
                                .to_pandas()
                    )
