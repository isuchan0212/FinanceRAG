...
class BaseTask:
    ...
    def __init__(self, metadata: TaskMetadata, qrels = None):
        ...
        self.reranker = CohereReranker(api_key = os.getenv('CO_API_KEY')) #CohereReranker(api_key=os.getenv('CO_API_KEY')) #JinaReranker(api_key=JINA_API_KEY)#
...
# Query에서 Keyword 뽑기 
    ...
    def hybrid_retrieve_rerank(self, top_k: int = 100, query_ids: Optional[List[str]] = None, **kwargs) -> Dict[str, Dict[str, float]]:
        ...
        for q_id, query in tqdm(self.queries.items(), desc="Hybrid search", total=len(self.queries)):
            ...
            # 예제 ID 리스트 (numpy array)
            exclude_ids = []
            # retrieved docs를 합치기 위함
            all_retrieved_docs = pandas.DataFrame()
            # 필터링 조건
            # hybrid search with original query after filtering out the docs that failed in step 1
            for i in trange(0, 10, desc='iterating for 10 times'):
                print(exclude_ids)
                if not exclude_ids:
                    try:
                        retrieved_docs = (self.hybrid_retriever
                                    .search(query=query, query_type='hybrid')
                                    .where(f"find_id = '{q_id[1:5]}' ", prefilter=True).limit(top_k)
                                    .rerank(reranker=self.reranker).limit(10).rerank(reranker=self.reranker).limit(1)
                                    .to_pandas()
                            )
                        all_retrieved_docs = pandas.concat([all_retrieved_docs, retrieved_docs], ignore_index=True)
                        if not retrieved_docs.empty and 'doc_id' in retrieved_docs.columns:
                            exclude_ids.append(retrieved_docs['doc_id'].iloc[0])  # 첫 번째 doc_id 값에 접근
                        ...
                        all_retrieved_docs = pandas.concat([all_retrieved_docs, retrieved_docs], ignore_index=True)
                        if not retrieved_docs.empty and 'doc_id' in retrieved_docs.columns:
                            exclude_ids.append(retrieved_docs['doc_id'].iloc[0]) 
                else:
                    condition = f"doc_id NOT IN ({', '.join(f'{id_}' for id_ in exclude_ids)})"
                    try:
                        retrieved_docs = (self.hybrid_retriever
                                    .search(query=query, query_type='hybrid').where(condition, prefilter = True)
                                    .where(f"find_id = '{q_id[1:5]}' ", prefilter=True).limit(top_k)
                                    .rerank(reranker=self.reranker).limit(15).rerank(reranker=self.reranker).limit(1)
                                    .to_pandas()
                            )
                        all_retrieved_docs = pandas.concat([all_retrieved_docs, retrieved_docs], ignore_index=True)
                        if not retrieved_docs.empty and 'doc_id' in retrieved_docs.columns:
                        ...
                        #logger.warning(f"query {q_id} failed with hybrid search: {e}. Retrying with vector search only.")
                        retrieved_docs = (self.hybrid_retriever
                                        .search(query=query, query_type='vector').where(condition, prefilter = True).limit(top_k)
                                        .rerank(reranker=self.reranker).limit(15).rerank(reranker=self.reranker).limit(1)
                                        .to_pandas()
                            )
                        all_retrieved_docs = pandas.concat([all_retrieved_docs, retrieved_docs], ignore_index=True)
